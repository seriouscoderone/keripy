...
_hab = None
_parser = None
# Dedicated lax+non-local Kevery+Parser for inbound POST traffic. Set up in
# init() — see comment there for the rationale (kerihost issue #6 analysis).
_external_kvy = None
_external_parser = None
...
def init():
    ...
    # Dedicated lax+non-local Kevery + Parser for inbound POST traffic.
    # Matches keri.app.indirecting.setupWitness (the keripy reference
    # witness binary) which uses Kevery(lax=True, local=False) for
    # third-party event processing. The Habery default _hby.psr / _hby.kvy
    # uses local=True, which is correct for locally-authored messages
    # (e.g. the self-rpy publication above) but the wrong polarity for
    # events arriving from external controllers. See kerihost issue #6
    # for the broader catchup-by-new-witness scenario; this swap is
    # hygiene-level (matches the canonical pattern) but does not on its
    # own fix the deeper Kevery escrow gap that #6 surfaces.
    global _external_kvy, _external_parser
    from hio.help import decking
    from keri.core import Kevery, parsing, routing
    from keri.kering import Vrsn_1_0
    _ext_cues = decking.Deck()
    _ext_rvy = routing.Revery(db=_hby.db, cues=_ext_cues)
    _external_kvy = Kevery(
        db=_hby.db, lax=True, local=False, rvy=_ext_rvy, cues=_ext_cues,
    )
    _external_kvy.registerReplyRoutes(router=_ext_rvy.rtr)
    _external_parser = parsing.Parser(
        framed=True, kvy=_external_kvy, rvy=_ext_rvy,
        exc=_hby.exc, version=Vrsn_1_0,
    )
...
def _drain_receipt_cues(kvy, hab):
    ...
    Args:
        kvy (Kevery): the Kevery whose cues queue we drain. For inbound
            external traffic this is `_external_kvy` (lax=True, local=False)
            rather than `_hby.kvy` (local=True), so that the receipt cues
            generated by the inbound parser end up in the right queue.
        hab (Hab): the witness Hab used to sign receipts.
    ...
    while True:
        ...
        while kvy.cues:
            ...
            if serder is None:
                continue
            kever = kvy.kevers.get(serder.pre)
            if kever is None:
...
def handle_cesr_ingest(event):
    ...
    _external_parser.parse(ims=ims, framed=True)
    _external_kvy.processEscrows()
    receipts = _drain_receipt_cues(_external_kvy, _hab)
    if receipts:
        ...
        _external_parser.parse(ims=bytearray(receipts))
