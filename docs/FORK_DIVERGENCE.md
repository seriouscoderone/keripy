...
| `src/keri/peer/exchanging.py` | `serializeMessage` no longer seeds its attachment accumulator with `exn.raw`. Upstream put the JSON body into `atc` and then applied two CESR-attachment rules to it: the `len(atc) % 4` quadlet check (so it raised on ~3 exns in 4 — measured live as `ValueError: Invalid attachments size=489, nonintegral quadlets.` on every IPEX admit-back), and, for `framed=False`, an AttachmentGroup counter sized over body+attachments and emitted BEFORE the body. `framed=True` output is byte-identical after the fix, it just stops raising; `framed=False` becomes `[body][counter][attachments]`, which also makes a caller that strips `exn.size` off the front land on the counter rather than mid-body (locksmith's group-multisig admit path does exactly that). Also returns a bare `None` for a missing exn instead of `(None, None)` — a 2-tuple is truthy, so every caller's `if not atc:` guard sailed past the not-found case. Upstream had **no tests** for this function; `tests/peer/test_serialize_message.py` adds five, all of which fail against the unpatched original |
